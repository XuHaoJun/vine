  ...
  return {
    async getCreatorPayoutOverview(input: { userId: string }) {
      ...
      const availableNetAmountMinor = overview.availableLedgers.reduce(
        (sum: number, row: any) => sum + row.netAmountMinor, 0,
      )
      return {
        availableNetAmountMinor,
        currency: 'TWD' as const,
        bankAccount: overview.account
          ? { id: overview.account.id, bankName: overview.account.bankName, accountLast4: overview.account.accountLast4 }
          : undefined,
    ...
    async requestCreatorPayout(input: { userId: string }) {
      ...
      const grossAmountMinor = overview.availableLedgers.reduce(
        (sum: number, row: any) => sum + row.grossAmountMinor, 0,
      )
      const taxWithholdingMinor = overview.availableLedgers.reduce(
        (sum: number, row: any) => sum + row.taxWithholdingMinor, 0,
      )
      const transferFeeMinor = overview.availableLedgers.reduce(
        (sum: number, row: any) => sum + row.transferFeeMinor, 0,
      )
    ...
    async upsertCreatorPayoutAccount(input: {
      ...
      const overview = await deps.repo.getCreatorPayoutOverview(deps.db, { userId: input.userId })
      ...
      return { id: account.id, bankName: account.bankName, accountLast4: account.accountLast4 }
    },
    ...
    approveRequest(input: { actorUserId: string; requestId: string }) {
      return deps.repo.approveRequest(deps.db, { ...input, now: deps.now().toISOString() })
    },
    ...
    createBatch(input: { actorUserId: string; requestIds: string[] }) {
      return deps.repo.createBatchFromApprovedRequests(deps.db, {
        id: deps.createId(), ...input, now: deps.now().toISOString(),
      })
    },
    ...
    markPaid(input: { actorUserId: string; requestId: string; bankTransactionId: string; paidAt: string }) {
      return deps.repo.markRequestPaid(deps.db, { ...input, now: deps.now().toISOString() })
    },

    markFailed(input: { actorUserId: string; requestId: string; reason: string }) {
      return deps.repo.markRequestFailed(deps.db, { ...input, now: deps.now().toISOString() })
    },
  }
}
...
function encodeCsv(rows: ManualPayoutCsvRow[]) {
  const headers = [
    'batchId', 'payoutRequestId', 'creatorId', 'creatorDisplayName', 'legalName',
    'bankCode', 'bankName', 'branchName', 'accountNumber', 'accountLast4',
    'currency', 'grossAmountMinor', 'taxWithholdingMinor', 'transferFeeMinor', 'netAmountMinor', 'memo',
  ] as const
  const lines = [
    headers.join(','),
