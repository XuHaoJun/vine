...
export function ReviewResult({ packageId }: { packageId: string }) {
  ...
  const [pkg] = useZeroQuery(
    ...
    { packageId, creatorId },
    { enabled: Boolean(packageId) && Boolean(creatorId) },
  )
  ...
  return (
    <YStack flex={1} p="$4" gap="$4">
      <SizableText size="$6" fontWeight="700">{pkg.name}</SizableText>

      {pkg.status === 'in_review' && (
        <YStack gap="$2" p="$3" bg="$color2" rounded="$4">
          <SizableText size="$4" fontWeight="600">審核中</SizableText>
          <SizableText>
            提交日期：{pkg.submittedAt ? new Date(pkg.submittedAt).toLocaleDateString('zh-TW') : '—'}
          </SizableText>
      ...
      {pkg.status === 'rejected' && (
        <YStack gap="$2" p="$3" bg="$color2" rounded="$4">
          ...
          {pkg.reviewProblemAssetNumbers && (
            <XStack gap="$1" flexWrap="wrap">
              <SizableText>問題素材編號：</SizableText>
              {pkg.reviewProblemAssetNumbers.split(',').map((n) => (
      ...
      {pkg.status === 'approved' && (
        <YStack gap="$2" p="$3" bg="$color2" rounded="$4">
          ...
          <Button
            ...
            onPress={() => publish.mutate(packageId)}
          >
            立即上架
          </Button>
        </YStack>
      ...
      {pkg.status === 'draft' && (
        <YStack gap="$2" p="$3" bg="$color2" rounded="$4">
          <SizableText size="$4" fontWeight="600">草稿</SizableText>
