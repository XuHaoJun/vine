/**
 * auto-generated by: on-zero generate
 *
 * grouped query re-exports for minification-safe query identity.
 * this file re-exports all query modules - while this breaks tree-shaking,
 * queries are typically small and few in number even in larger apps.
 */
export * as chat from '../queries/chat'
export * as chatOaLoading from '../queries/chatOaLoading'
export * as creatorProfile from '../queries/creatorProfile'
export * as entitlement from '../queries/entitlement'
export * as friendship from '../queries/friendship'
export * as message from '../queries/message'
export * as oaAudienceFilter from '../queries/oaAudienceFilter'
export * as oaCampaign from '../queries/oaCampaign'
export * as oaChatFilter from '../queries/oaChatFilter'
export * as oaContactTag from '../queries/oaContactTag'
export * as oaFriendship from '../queries/oaFriendship'
export * as stickerAsset from '../queries/stickerAsset'
export * as stickerPackage from '../queries/stickerPackage'
export * as todo from '../queries/todo'
export * as user from '../queries/user'
